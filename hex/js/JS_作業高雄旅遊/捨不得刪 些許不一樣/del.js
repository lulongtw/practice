...
window.onload = async () => {
  try {
    ...
    for (item of res) {
      let distName = codeToName(item.Zipcode);
      if (!data[distName]) {
        data[distName] = [];
        data[distName].push(item);
      } else {
        data[distName].push(item);
      }
    }
...
function refreshMain(dist, page) {
  ...
  for (let i = page - 8; i < page && i < data[dist].length; i++) {
    ...
    <div class="des">
      <div class="smallPic"><img src="./assets/icons_clock.png"></div>
      <div class="time">${data[dist][i].Opentime}</div>
      <div class="smallPic"><img src="./assets/icons_pin.png"></div>
      <div class="location">${data[dist][i].Add}</div>
      <div class="smallPic"><img src="./assets/icons_phone.png"></div>
      <div class="phone">${data[dist][i].Tel}</div>
      <div class="smallPicLast"><img src="./assets/icons_tag.png"></div>
