<script setup>
  ...
  function send(){
    ...
    noCatch(axios.post(url,userData.value)
      .then(res=>{
        ...
        document.cookie = `greenToken=${token};expires=${new Date(expired)}`
  ...
  onMounted(()=>{
    let fromLocal = localStorage.getItem("remItem")
    if (fromLocal){
...
<main class="form-signin w-100 m-auto">
  ...
  <h1 class="h3 mb-3 fw-normal">Please sign in</h1>
    ...
    <div class="form-check text-start my-3">
      <input class="form-check-input" type="checkbox" v-model="remember" id="flexCheckDefault">
      <label class="form-check-label" for="flexCheckDefault">
        Remember me
      </label>
    </div>
    <button class="btn btn-primary w-100 py-2" type="submit">Sign in</button>
    <p  @click='router.push("/dashboard/simCustomer")' class="mt-5 mb-3 text-body-secondary">go to 模擬訂單</p>
    <router-link to="/dashboard/simCustomer">也是go to 模擬訂單 媽的腦袋爆炸</router-link>
  </form>
</main>
