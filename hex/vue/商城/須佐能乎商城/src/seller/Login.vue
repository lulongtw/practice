<script setup>
  ...
  onMounted(()=>{
    if (localStorage.getItem("cough")){
      obj.value = JSON.parse(localStorage.getItem("cough"))
    }
  })
  ...
  async function checkUser(){
    ...
    if (rememberMe.value){
      localStorage.setItem('cough',JSON.stringify(obj.value))
    }
    let res = await store.dispatch('checkUser',{url,method,toSend})
  }
